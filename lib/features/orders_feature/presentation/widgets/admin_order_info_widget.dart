...
class AdminOrderInfoWidget extends StatelessWidget {
  ...
  final bool pop;
  const AdminOrderInfoWidget({Key key, this.order, this.pop}) : super(key: key);
          ...
          child: Wrap(
            ...
            children: [
              LabelRow(
                ...
                onTap: () => Services.makePhoneCall(order.user.phone),
              ),
              ...
              if (Services.hasRole(context, operationManagerRole))
                MediaIcon(icon: FontAwesomeIcons.whatsapp, url: 'customer_whatsapp', mobileNumber: order.user.phone),
              if (Services.hasRole(context, operationManagerRole))
                InkWell(
