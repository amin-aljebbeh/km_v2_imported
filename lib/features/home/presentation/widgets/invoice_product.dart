    ...
    return StoreConnector<AppState, AppState>(
      ...
      builder: (context, state) {
        return Padding(
          ...
          child: KCard(
            onTap: () =>
                Navigator.push(context, MaterialPageRoute(builder: (context) => ProductDetailView(product: product))),
              ...
              child: Center(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      StringUtils().oCcy.format(int.parse(product.price.split('.')[0]) + (product.discountValue ?? 0)) +
                          ' ' +
                          state.generalInformationState.companyInformation.currency,
                      textAlign: TextAlign.right,
                      style: invoiceProductStyle.copyWith(
                          decoration:
                              ((product.discountValue ?? 0) != 0) ? TextDecoration.lineThrough : TextDecoration.none,
                      ...
                      if (product.discountValue != 0)
                        Text(
                          StringUtils().oCcy.format(int.parse(product.price.split('.')[0])) +
                              ...
                              state.generalInformationState.companyInformation.currency,
                          textAlign: TextAlign.right,
                          style: invoiceProductStyle.copyWith(color: kmRed),
                        ),
                    Text(
